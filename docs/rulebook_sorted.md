...
A zone is a two-dimensional space. created via zoning process by converting enclosed spaces, such as rooms, halls, corridors, using convex decomposition as orientation tool, into self contained areas. in order to create movement lines that connect all portals within that zone as unhindered segments. Zones partition the plan. Every point of the building belongs to exactly one zone, without gaps or overlaps. Every zone has at least one portal.
...
A portal is the crossable portion of a boundary between two zones. Physically an opening is a line on a floor plan, here it is modelled as a single point at midpoint, which ensures the possibility of exactly one segment per portal pair per zone.
...
Every portal joins exactly two zones. No exceptions.

Where three corridors meet, the shared point is not a portal. Each pair of corridors gets its own portal at the midpoint of the boundary those two share, as if the third were not there — three portals close together, each joining two zones.
...
Outside the building is one zone, the exterior, encircling it. It is never crossed or used in a route, no movement zones or lines lie in the exterior. It may be a start or a target. Entrances are portals between it and the zones they open onto.
...
The connectivity graph is derived from the adjacency graph by zoning. Zoning replaces rooms with zones: one room may become several zones, and several rooms may be absorbed into one. walls become boundaries between them.

### 3.5

The routing graph is built from the connectivity graph. Each portal becomes a node; two portals are linked when they lie on the same zone, and the link is the segment between them.
...
The result of a query is a chain of portals. The zone each segment lies in gives the sequence of zones crossed; the order of a zone's portals along its movement line gives each portal's position within that zone. Together these produce the instructions.
